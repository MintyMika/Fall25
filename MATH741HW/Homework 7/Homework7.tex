...
\begin{proof}
    ...
    \end{align*}
    where $A = (a_{ij})$ is the matrix whose columns are the coordinates of $v_i$ in the basis $\{ e_1, \dots, e_n \}$. Now, note that $\beta(\phi(e_1), \phi(e_2), \dots, \phi(e_n))$ is an $n$-linear antisymmetric form evaluated at the basis vectors $\{ e_1, \dots, e_n \}$ after applying $\phi$. By the definition of determinant, we have
    \[
...
\begin{proof}
    Let $\{ E_{ij} \mid 1 \leq i, j \leq n \}$ be the standard basis of $V$, where $E_{ij}$ is the matrix with a $1$ in the $(i,j)$-th position and $0$ elsewhere. Then we have 
    \[
        m_M(E_{ij}) = M E_{ij} = \text{the } j\text{-th column of } M \text{ placed in the } i\text{-th column of a zero matrix}.
    \]
    Thus, the action of $m_M$ on the basis elements can be described as follows:
    \[
        m_M(E_{ij}) = \sum_{k=1}^n M_{ki} E_{kj}.
    \]
    Therefore, the matrix representation of $m_M$ with respect to this basis is given by a block matrix where each block corresponds to the multiplication by the columns of $M$. Specifically, the matrix representation can be viewed as an $n^2 \times n^2$ matrix where each block is an $n \times n$ matrix formed by the columns of $M$. 

    The determinant of this block matrix can be computed as follows:
    \[
        \det(m_M) = (\det(M))^n,
    \]
    since each of the $n$ blocks contributes a factor of $\det(M)$ to the overall determinant.
