...
\begin{proof}
    To prove the 'inverse Schur Lemma', we need to show that if every $G$-equivariant map $\phi : V \to V$ is scalar, then $V$ is irreducible. We will assume that the field is algebraically closed ($K = \overline{K}$).
...
\begin{proof}
    ...
    To show that $\phi$ is a representation isomorphism, we need to verify two things that $\phi$ is linear and $\phi$ commutes with the action of $S_n$.

    First we show that $\phi$ is linear. For any $f_1, f_2 \in V_k$ and scalars $a, b \in \cc$, we have
    \[
    ...
    Next, we show that $\phi$ commutes with the action of $S_n$. For any $\sigma \in S_n$, we need to show that
    \[
...
\begin{proof}
    ...
    \textbf{Base case:} For $k = 0$, we have $V_0$ is the space of functions on the empty set, which is one-dimensional. Thus $V_0 = W_0$ where $W_0$ is the trivial representation of $S_n$.

    \textbf{Inductive step:} Assume that for some $k < \lfloor n/2 \rfloor$, we have
    \[
    V_k \cong \bigoplus_{i=0}^{k} W_i
    \]
    where $W_0, W_1, \ldots, W_k$ are irreducible representations of $S_n$. We need to show that
    \[
    V_{k+1} \cong \bigoplus_{i=0}^{k+1} W_i.
    \]

    Consider the restriction of $V_{k+1}$ as a representation of $S_n$. We can write $V_{k+1}$ as a sum of irreducible representations:
    \[
    V_{k+1} = \bigoplus_{j} U_j^{(k+1)}
    \]
    where the $U_j^{(k+1)}$ are irreducible.

    By the previous problem, we know that $V_{k+1} \cong V_{n-(k+1)}$. Therefore, we can also write
    \[
    V_{n-(k+1)} = \bigoplus_{j} U_j^{(n-(k+1))}
    \]
    where the $U_j^{(n-(k+1))}$ are irreducible. Since $V_{k+1} \cong V_{n-(k+1)}$, we have
    \[
    \bigoplus_{j} U_j^{(k+1)} \cong \bigoplus_{j} U_j^{(n-(k+1))}.
    \]
    By the induction hypothesis, we know that $V_k \cong \bigoplus_{i=0}^{k} W_i$. Therefore, we can write
    \[
    V_{k+1} = V_k \oplus U^{(k+1)}
    \]
    where $U^{(k+1)}$ is an irreducible representation of $S_n$. This is because $V_{k+1}$ is a direct sum of irreducible representations, and $V_k$ is already a direct sum of irreducible representations. Thus, the remaining part must be an irreducible representation. Therefore, we have
    \[
    V_{k+1} \cong \bigoplus_{i=0}^{k} W_i \oplus U^{(k+1)}.
    \]
    Since $U^{(k+1)}$ is an irreducible representation, we can set $W_{k+1} = U^{(k+1)}$. Therefore, we have
    \[
    V_{k+1} \cong \bigoplus_{i=0}^{k+1} W_i.
    \]
